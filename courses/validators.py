...
def validate_image_dimensions(value):
    ...
    try:
        ...
        if width < MIN_IMAGE_WIDTH or height < MIN_IMAGE_HEIGHT:
            raise ValidationError(
                _(
                    "Image dimensions too small. Minimum %(min_width)sx%(min_height)spx. "
        ...
        if width > MAX_IMAGE_WIDTH or height > MAX_IMAGE_HEIGHT:
            raise ValidationError(
                _(
                    "Image dimensions too large. Maximum %(max_width)sx%(max_height)spx. "
                    "Got: %(width)sx%(height)spx"
                ),
                params={
...
def education_flag_path(instance, filename):
    ...
    if ext not in ALLOWED_IMAGE_EXTENSIONS:
        raise ValidationError(
            _(
                "Invalid file extension: %(ext)s. Allowed: %(allowed)s"
            ),
            params={
                'ext': ext,
