        ...
        if self.country_code:
            ...
            if len(self.country_code) != 3 or not self.country_code.isalpha():
                raise ValidationError(
                    {
                        "country_code": _(
                            "Country code must be 3 letters (ISO 3166-1 alpha-3)"
                        )
                    }
                )
...
class Subject(models.Model):
    ...
    name_ar = models.CharField(
        max_length=100, blank=True, null=True, help_text=_("Subject name in Arabic")
    )
    ...
    description = models.TextField(
        blank=True, help_text=_("Subject description and scope")
    )
...
class Course(models.Model):
    ...
    title = models.CharField(max_length=200, db_index=True, help_text=_("Course title"))
    ...
    published_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When course was first published")
    )

    deleted_at = models.DateTimeField(
        null=True, blank=True, help_text=_("Timestamp when course was soft-deleted")
    )
    ...
    def total_revenue(self):
        ...
        """
        result = self.course_enrollments.filter(status="active").aggregate(
    ...
    def soft_delete(self):
        ...
        self.deleted_at = timezone.now()
        self.save(
            update_fields=["is_active", "is_published", "deleted_at", "updated_at"]
        )
