...
from users.permissions import (
    ...
    IsStudent,
    IsEnrollmentOwner,
    IsLessonCourseTeacherOrReadOnly,
)
...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    #     enrollment = Enrollment.objects.filter(course=course, student=user).first()
    #     if not enrollment:
    #         return Response({'detail': 'You are not enrolled in this course.'}, status=status.HTTP_400_BAD_REQUEST)

    #     # Delete the enrollment
    #     enrollment.delete()
    #     return Response({'detail': 'Unenrollment successful.'}, status=status.HTTP_200_OK)
