...
class TeacherSerializerTests(APITestCase):
    ...
    def test_create_teacher(self):
        ...
        self.assertTrue(
            check_password(data["user_info"]["password"], teacher.user.password)
        )
    ...
    def test_update_teacher(self):
        ...
        serializer = teacherCreateUpdateSerializer(
            instance=self.teacher, data=data, partial=True, context={"request": request}
        )
    ...
    def test_user_type_read_only(self):
        ...
        request = factory.patch("/")
        request.user = self.teacher_user
        data = {"user_info": {"user_type": "admin"}}
        serializer = teacherCreateUpdateSerializer(
            instance=self.teacher, data=data, partial=True, context={"request": request}
        )
        self.assertTrue(serializer.is_valid())
...
class StudentSerializerTests(APITestCase):
    def setUp(self):
        self.student_user = User.objects.create_user(
            username="student1", password="password123", user_type="student"
        )
        self.student = Student.objects.create(
            user=self.student_user,
            academic_year=3,
            phone="01234557890",
            parent_phone="01234547891",
        )
    ...
    def test_create_student(self):
        ...
        self.assertTrue(
            check_password(data["user_info"]["password"], student.user.password)
        )
    ...
    def test_update_student(self):
        ...
        serializer = StudentCreateUpdateSerializer(
            instance=self.student, data=data, partial=True, context={"request": request}
        )
        self.assertTrue(serializer.is_valid())
        student = serializer.save()
        self.assertEqual(student.user.username, data["user_info"]["username"])
    ...
    def test_user_type_read_only(self):
        ...
        request = factory.patch("/")
        request.user = self.student_user
        data = {"user_info": {"user_type": "teacher"}}
        serializer = StudentCreateUpdateSerializer(
            instance=self.student, data=data, partial=True, context={"request": request}
        )
        self.assertTrue(serializer.is_valid())
