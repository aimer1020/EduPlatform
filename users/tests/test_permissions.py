...
class PermissionTests(TestCase):
    ...
    def setUp(self):
        ...
        temp_img = SimpleUploadedFile(
            ...
            content=file_io.read(), content_type='image/jpeg')
        # Create users
        self.admin_user = User.objects.create_user(username='admin', user_type='admin', is_staff=True, is_superuser=True)
        self.teacher_user = User.objects.create_user(username='teacher1', user_type='teacher')
        self.other_teacher_user = User.objects.create_user(username='teacher2', user_type='teacher')
        self.student_user = User.objects.create_user(username='student1', user_type='student')
        self.other_student_user = User.objects.create_user(username='student2', user_type='student')

        # Create teacher profiles
        self.teacher = Teacher.objects.create(user=self.teacher_user, is_verified=True)
        self.other_teacher = Teacher.objects.create(user=self.other_teacher_user, is_verified=True)

        # Create student profiles
        self.student = Student.objects.create(user=self.student_user, phone='01234567890', parent_phone='01012345678')
        self.other_student = Student.objects.create(user=self.other_student_user, phone='01012345676', parent_phone='01122334455')
    ...
    def test_is_enrollment_owner(self):
        ...
        self.assertFalse(permission.has_object_permission(request, None, self.enrollment))
